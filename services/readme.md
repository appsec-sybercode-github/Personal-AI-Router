...
[vLLM](https://docs.vllm.ai/), [SGLang](https://docs.sglang.ai/), any combination
of them, or none, and its model inventory is fetched over HTTP from its
engine-manager rather than crammed into mDNS TXT records, which are too small to
carry it.
...
| `lmstudio-proxy` | OpenAI-compatible counterpart to `ollama-proxy`, forwarding OpenAI-compatible inference routes for **every** such engine — LM Studio, vLLM and SGLang — with equivalent owner-only routing and failover behavior. One process fronts them all; the binary name is a wire contract and keeps its historical spelling. |
...
lmstudio-proxy/          OpenAI-compatible routing proxy (LM Studio, vLLM, SGLang)
